...
export function attachSliderFunctions(grahpicsElement) {
  const __sliders = {};

  function addSlider(propname, options = {}) {
    const {
      min = 0,
      max = 1,
      step = 1,
    ...
    // TODO: figure out how to find the right place to inject this,
    // since the graphics code itself only knows about the __canvas

    setSlider(slider, propname, value, transform);
  }
   ...
   * @param {*} value
   */
  function updateSlider(propname, min, max, step, value) {
    let slider = __sliders[propname];
   ...
   */
  function setSlider(qs, propname, initial, transform) {
  ...
  return { addSlider, setSlider, updateSlider, removeSliders };
}
